...
def predict_one(path, model, mask_outdir, overlay_outdir, device):
    ...
    prediction = predict_pil(
        model, img_pil, model_dims=(416, 416), device=device)
...
def predict_pil(model, img_pil, model_dims, device):
    ...
    prediction = myutils.postprocessing_pred(
        prediction.squeeze().cpu().round().numpy().astype(np.uint8))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='V-FloodNet: Water Image Segmentation')
    ...
    test_waterseg(args.model_path, args.test_path,
                  args.test_name, args.out_path, device)
