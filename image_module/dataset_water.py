    ...
    def __init__(self, mode, dataset_path, input_size=None, test_case=None, eval_size=None):
        ...
        if mode == 'train_offline':
            ...
            for sub_folder in water_subdirs:
                label_list = glob(os.path.join(
                    dataset_path, 'Annotations/', sub_folder, '*.png'))
                label_list.sort(key=lambda x: (len(x), x))
                self.label_list += label_list

                name_list = [os.path.basename(x)[:-4] for x in label_list]

                img_list = glob(os.path.join(
                    dataset_path, 'JPEGImages/', sub_folder, '*.jpg'))
            ...
            first_frame_label_path = os.path.join(
                dataset_path, 'Annotations/', test_case, img_list[0])
            ...
            if not os.path.exists(first_frame_label_path):
                label_list = glob(os.path.join(
                    dataset_path, 'Annotations/', test_case, '*.png'))
            ...
            self.first_frame_label = load_image_in_PIL(
                first_frame_label_path, 'P')

            if self.eval_size:
                self.origin_size = self.first_frame.size
                self.first_frame = self.first_frame.resize(
                    self.eval_size, Image.ANTIALIAS)
                self.first_frame_label = self.first_frame_label.resize(
                    self.eval_size, Image.ANTIALIAS)

        else:
            raise (
                'Mode %s does not support in [train_offline, train_online, eval].' % mode)
...
class WaterDataset_RGB(WaterDataset):
    def __init__(self, mode, dataset_path, input_size=None, test_case=None, eval_size=None):
        super(WaterDataset_RGB, self).__init__(
            mode, dataset_path, input_size, test_case, eval_size)
    ...
    def apply_transforms(self, img, label=None):
        if self.mode == 'train_offline' or self.mode == 'train_online':
            ...
            img, label = my_tf.random_affine_transformation(
                img, None, label, self.verbose_flag)
            img, label = my_tf.random_resized_crop(
                img, None, label, self.input_size, self.verbose_flag)
