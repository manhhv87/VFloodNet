...
def random_affine_transformation(img, mask, label, verbose=False):
    ...
    if random.random() < random_thres:
        ...
        img = TF.affine(img, degrees, (translate_h, translate_v),
                        scale, shear, resample)
        if mask:
            mask = TF.affine(mask, degrees, (translate_h,
                             translate_v), scale, shear, resample)
        label = TF.affine(label, degrees, (translate_h,
                          translate_v), scale, shear, resample)
...
def random_mask_perturbation(mask, verbose=False):
    ...
    mask = TF.affine(mask, degrees, (translate_h, translate_v),
                     scale, shear, resample)
...
def random_resized_crop(img, mask, label, size, verbose=False):
    ...
    if mask:
        mask = TF.resized_crop(mask, y, x, h, w, size,
                               TF.InterpolationMode.BICUBIC)
    label = TF.resized_crop(label, y, x, h, w, size,
                            TF.InterpolationMode.BICUBIC)
