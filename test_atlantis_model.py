...
def main(args):
    ...
    test_dataset = WaterDataset(
        args.data_directory, split="test", padding_size=args.padding_size)
    ...
    with torch.no_grad():
        for image, mask, name, width, height in tqdm(test_dataloader):
            ...
            pred = interpolation(pred).detach().cpu().numpy()[
                0].transpose(1, 2, 0)
            ...
            cv2.imwrite('%s/%s_water.png' %
                        (args.save_path, name[0][:-4]), overlay)
...
def get_arguments(
    ...
    parser = argparse.ArgumentParser(
        description=f"Testing {model} on ATLANTIS 'test' set.")
