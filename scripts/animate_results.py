...
def cvt_images_to_video(img_dir,
    ...
    for i in trange(1, len(img_list)):
        ...
        k = max(1, bisect.bisect_left(
            timestamp_list_gt, timestamp_list_est[i]))
        ax.plot(timestamp_list_gt[:k], gt_val[:k], '^',
                markersize=markersize, label=f'Groundtruth')
        if 'houston' in args.video_name:
            ax.axhline(y=10.3, linestyle='--')
            ax.plot(timestamp_list_est[:i], est_val[:i], 'o',
                    markersize=markersize // 4, label=f'Estimated {type}')
        else:
            ax.plot(timestamp_list_est[:i], est_val[:i], 'o',
...
if __name__ == '__main__':
    ...
    data_path = os.path.join(
        args.viz_dir, f'{args.video_name}_ref', 'waterlevel.csv')
    ...
    cvt_images_to_video(img_dir, viz_dir, data_path,
                        gt_path, video_path, fps=fps)
