...
def run_inference(args, image_list, _metro_network, smpl, renderer, mesh_sampler):
    ...
    for idx, image_file in enumerate(tqdm(image_list, desc='Est People Mesh')):
        if 'pred' not in image_file:
            ...
            pred_3d_pelvis = pred_3d_joints_from_smpl[:, cfg.H36M_J17_NAME.index(
                'Pelvis'), :]
            ...
            pred_2d_joints_from_smpl = orthographic_projection(
                pred_3d_joints_from_smpl, pred_camera)
            pred_2d_431_vertices_from_smpl = orthographic_projection(
                pred_vertices_sub2, pred_camera)
            ...
            cv2.imwrite(temp_fname, np.asarray(
                visual_imgs[:, -225:, ::-1] * 255))

            pred_2d = pred_2d_431_vertices_from_smpl[0].detach(
            ).cpu().numpy().tolist()
...
def parse_args():
    ...
    parser.add_argument("--hidden_feat_dim", default='1024,256,128', type=str,
                        help="The Image Feature Dimension.")
    parser.add_argument("--legacy_setting", default=True,
                        action='store_true', )
    #########################################################
    # Others
    #########################################################
...
def main(args):
    ...
    if args.resume_checkpoint != None and args.resume_checkpoint != 'None' and 'state_dict' not in args.resume_checkpoint:
        ...
        hidden_feat_dim = [int(item)
                           for item in args.hidden_feat_dim.split(',')]
        ...
        logger.info('Backbone total parameters: {}'.format(
    ...
    if op.isfile(args.image_file_or_path):
        ...
        raise ValueError("Cannot find images at {}".format(
            args.image_file_or_path))

    run_inference(args, image_list, _metro_network,
                  mesh_smpl, renderer, mesh_sampler)
