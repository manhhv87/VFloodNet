...
def main(args):
    ...
    if not os.path.exists(gt_csv_path):
        raise FileNotFoundError(
            'Please prepare the groundtruth file like gt.csv in ./records/groundtruth/<test_name>')
    ...
    if not os.path.exists(px_to_meter_path):
        raise FileNotFoundError(
            'Please prepare the conversion file like px_to_meter.txt in ./records/groundtruth/<test_name>')
    ...
    if 'boston_harbor' in args.test_name:
        timestamp_list_gt = pd.to_datetime(
            gt_csv.iloc[:, 0] + ' ' + gt_csv.iloc[:, 1])
        ...
        timestamp_list_gt = pd.to_datetime(
            gt_csv.iloc[:, 0], format='%m/%d/%Y %H:%M')
        gt_col_id = 2
        ticker_locator = mdates.HourLocator(interval=6)
        type = 'Water Level'
    elif 'LSU' in args.test_name:
        timestamp_list_gt = pd.to_datetime(
            gt_csv.iloc[:, 0], errors='coerce', format='%Y-%m-%d-%H-%M-%S')
        gt_col_id = 1
        type = 'Water Depth'
    ...
    for i in range(tracker_num):
        waterlevel_meter[i] = px_to_meter[i, 0] * \
            waterlevel[f'est_ref{i}_px'] + px_to_meter[i, 1]
    ...
    # Calc error
    gt_val_sample = get_gt_sample(
        timestamp_list_est, timestamp_list_gt, gt_val)
    ...
    ax.plot(timestamp_list_gt, gt_val, '^',
            markersize=markersize, label=f'Groundtruth')
    ...
    if 'houston' in args.test_name:
        ...
        ax.text(timestamp_list_est[-1000], high_water_val, 'Observed High Water Mark',
                va='center', ha='center', backgroundcolor='w', fontsize=fontsize)
        ax.legend(loc='upper right', fontsize=fontsize)
    else:
        ax.plot(timestamp_list_est, waterlevel[metric], 'o',
                markersize=markersize, label=f'Estimated {type}')
