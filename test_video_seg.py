...
def get_args():
    parser = argparse.ArgumentParser(
        description='V-FloodNet: Water Video Segmentation')
...
def main(args, device):
    ...
    if not os.path.exists(mask_path):
        ...
        test_waterseg(image_model_path,
                      img_list[0], args.test_name, out_dir, device)
    ...
    seq_loader = utils.data.DataLoader(
        seq_dataset, batch_size=1, shuffle=False, num_workers=1)
    ...
    first_frame = TF.resize(
        ori_first_frame, downsample_size, InterpolationMode.BICUBIC)
    first_mask = TF.resize(ori_first_mask, downsample_size,
                           InterpolationMode.NEAREST)

    pred = torch.argmax(ori_first_mask[0],
    ...
    if args.viz:
        ...
        myutils.save_overlay(
            ori_first_frame[0], pred, overlay_path, myutils.color_palette)
    ...
    with torch.no_grad():
        ...
        for idx, (frame, frame_name) in enumerate(tqdm(seq_loader)):
            ...
            frame = TF.resize(ori_frame, downsample_size,
            ...
            if args.viz:
                ...
                myutils.save_overlay(
                    ori_frame[0], pred, overlay_path, myutils.color_palette)
